"""hash58: the iTunesDB header checksum for nano 3G / classic devices.

Port of libgpod's itdb_hash58.c (which is itself a port of wtbw's
proof-of-concept). The 20-byte hash sits at mhbd+0x58. It is an
HMAC-SHA1 over the whole DB with three header regions zeroed (db_id at
0x18, the unknown block at 0x32, and the hash slot itself), keyed by a
value derived from the device's FireWire GUID.

Verified against the real device: the DB the previous sync tool wrote
carries a hash that this implementation reproduces byte-for-byte (its
scheme byte is 0 rather than 0x58, which the hash covers either way).
We write 0x58, gtkpod's convention.
"""

from __future__ import annotations

import hashlib
import struct

# AES S-box and inverse S-box (same tables as libgpod).
_TABLE1 = bytes([
    ,
])
_TABLE2 = bytes([
    ,
])
_FIXED = bytes([
    0x67,0x23,0xFE,0x30,0x45,0x33,0xF8,0x90,0x99,0x21,0x07,0xC1,0xD0,0x12,0xB2,0xA1,0x07,0x81,
])


def _gcd(a: int, b: int) -> int:
    while True:
        a %= b
        if a == 0:
            return b
        b %= a
        if b == 0:
            return a


def _lcm(a: int, b: int) -> int:
    if a == 0 or b == 0:
        return 1
    return (a * b) // _gcd(a, b)


def _generate_key(firewire_id: bytes) -> bytes:
    """64-byte HMAC key from the 8-byte FireWire GUID."""
    y = bytearray(16)
    for i in range(4):
        a, b = firewire_id[i * 2], firewire_id[i * 2 + 1]
        cur = _lcm(a, b)
        hi, lo = (cur >> 8) & 0xFF, cur & 0xFF
        y[i * 4] = _TABLE1[hi]
        y[i * 4 + 1] = _TABLE2[hi]
        y[i * 4 + 2] = _TABLE1[lo]
        y[i * 4 + 3] = _TABLE2[lo]
    digest = hashlib.sha1(_FIXED + bytes(y)).digest()
    return digest + b"\x00" * 44


def _hmac_sha1(key: bytes, data: bytes) -> bytes:
    k = bytearray(key)
    for i in range(64):
        k[i] ^= 0x36
    inner = hashlib.sha1(bytes(k) + data).digest()
    for i in range(64):
        k[i] ^= 0x36 ^ 0x5C
    return hashlib.sha1(bytes(k) + inner).digest()


def compute_hash58(db: bytes, firewire_guid: str) -> bytes:
    """The 20 bytes that belong in the mhbd hash58 slot.

    @firewire_guid: the device's FireWireGUID, 16 hex chars
    (e.g. '000A27001BB9E492').
    """
    fwid = bytes.fromhex(firewire_guid)
    if len(fwid) != 8:
        raise ValueError(f"FireWireGUID must be 16 hex chars, got {firewire_guid!r}")

    buf = bytearray(db)
    # The three regions are zeroed for the computation, exactly like
    # libgpod does before writing the hash.
    buf[0x18:0x20] = b"\x00" * 8
    buf[0x32:0x46] = b"\x00" * 20
    buf[0x58:0x6C] = b"\x00" * 20
    return _hmac_sha1(_generate_key(fwid), bytes(buf))


def apply_hash58(db: bytes, firewire_guid: str) -> bytes:
    """Return @db with the scheme field set and the hash58 written.

    The scheme byte is part of the hashed content (libgpod sets it
    before computing), so it is written first.
    """
    out = bytearray(db)
    struct.pack_into("<H", out, 0x30, 0x58)          # hashing scheme
    out[0x58:0x6C] = compute_hash58(bytes(out), firewire_guid)
    return bytes(out)
