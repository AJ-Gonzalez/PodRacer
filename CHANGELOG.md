...
- Settings live in one fixed user-level file now (`~/.config/PodRacer/PodRacer.conf`, or `$XDG_CONFIG_HOME`), independent of where the app or its bundle lives, and identical across platforms. macOS migrates the old plist over once. (On macOS Qt's native-format store is unredirectable, which had let the test suite and headless probes write the real settings file.)
- Layout: both panes' columns auto-fill their space. Column shapes persist as ratios and scale to the window/pane on launch, resizes, and splitter moves; the rightmost column of both panes stays glued to the right edge, and dragging a boundary keeps it glued. Previously the file browser's columns could not fill the pane and a resized window left a dead gap.
